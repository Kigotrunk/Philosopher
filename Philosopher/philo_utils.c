...
void	ft_mutex(t_philo *philo)
{
	...
	printf("%d -> index -> %d\n", philo->identity + 1, (philo->identity));
    ft_print(0, philo);
    pthread_mutex_lock(&(philo->f[(philo->identity + 1) % philo->data->num_philo]));
	printf("%d -> index -> %d\n", philo->identity + 1, (philo->identity + 1) % philo->data->num_philo);
    ft_print(0, philo);
    ft_print(1, philo);
    ...
    pthread_mutex_unlock(&(philo->f[(philo->identity + 1) % philo->data->num_philo]));
	pthread_mutex_lock(&(philo->data_race));
	philo->n_meal++;
	pthread_mutex_unlock(&(philo->data_race));
}
